# Robot Feature Description Format (RFDF)

RFDF is a way for:

- Robot developers to define the various features present in their robot and describe ways to use them.
- Robot app developers to make their apps work on various robots and figure out if a particular robot has the necessary features to support the app.

RFDF uses [KDL](https://kdl.dev/).

## How To Use

Currently only ROS 2 is supported.

### For Robot Developers

Example ROS 2 package: [rfdf_example_ros2](./rfdf_example_ros2)

- Create a `rfdf.kdl` file and pass its path to the `rfdf_publisher` node through its `rfdf_file` parameter.
- The description will be automatically published on `robot_features` relative to the publisher node's namespace (`/robot_features` unless you launch the node under another namespace).

### For Robot App Developers

Subscribe to that fully-qualified topic (typically `/robot_features`) and use one of our parsers in your app's code to read the document. A relative subscription resolves under the subscriber's namespace.
